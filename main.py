...
IMG_FILE_PATH = 'backup/lucas/origa/ORIGA/Images'
CSV_FILE_PATH = '/backup/lucas/origa/ORIGA/OrigaList.csv'
...
def main():
    ...
    df = data_factory.load_data(verbose=True)
    if df is not None:
        print(df.head())
        print(f"Número total de imagens processadas: {len(df)}")        
        origa_dataset = ROPDataset(df, is_train=False, apply_clahe=True)
        X_train, y_train, train_indx, patient_ids_train, gkf, test_dataset = data_factory.prepare_data_for_cross_validation_3(origa_dataset, num_splits=5)

        # train_and_val_worker = TrainAndEvalWorker(config=None)
        # print("Iniciando o treinamento e validação com GroupKFold...")
        # train_and_val_worker.train(X_train, y_train, patient_ids_train, train_indx, gkf, origa_dataset)
        ...
        #     #Dividir em treino e teste
            ...
            #Utils.plot_sample_images(origa_dataset, num_samples=5)
